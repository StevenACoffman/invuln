...
// Otherwise, returns external.UnknownPackagePath.
func pkgPath(f *ssa.Function) string {
	...
	return external.UnknownPackagePath
}
...
func pkgModPath(pkg *packages.Package) string {
	...
	return external.UnknownModulePath
}

func IsStdPackage(pkg string) bool {
	if pkg == "" || pkg == external.UnknownPackagePath {
		return false
	}
	// std packages do not have a "." in their path. For instance, see
	// Contains in pkgsite/+/refs/heads/master/external/stdlbib/stdlib.go.
