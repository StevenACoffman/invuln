...
import (
	...
	"github.com/StevenACoffman/invuln/external/vulncheck"
)
...
const (
	detailsMessage = `For details, see https://pkg.go.dev/github.com/StevenACoffman/invuln/cmd/govulncheck.`
...
func (h *TextHandler) vulnerability(index int, findings []*findingSummary) {
	...
	for _, module := range byModule {
		...
		if stdPkg := h.pkg(module); path == external.GoStdModulePath && stdPkg != "" {
			path = stdPkg
		}
		...
		first = false
		h.print("  ")
		if mod == external.GoStdModulePath {
...
func isStdFindings(findings []*findingSummary) bool {
	for _, f := range findings {
		if vulncheck.IsStdPackage(f.Trace[0].Package) || f.Trace[0].Module == external.GoStdModulePath {
			return true
		}
	}
	return false
