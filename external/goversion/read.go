...
func ReadExe(file string) (Version, error) {
	...
	for _, sym := range syms {
		...
		if strings.Contains(name, "_Cfunc__goboringcrypto_") || strings.HasPrefix(name, "crypto/external/boring/sig.BoringCrypto") {
			v.BoringCrypto = true
		}
		if strings.HasPrefix(name, "crypto/external/boring/sig.FIPSOnly") {
			v.FIPSOnly = true
		}
		...
		if strings.HasPrefix(name, "crypto/external/boring/sig.StandardCrypto") {
			v.StandardCrypto = true
		}
	}
...
// Code signatures that indicate BoringCrypto or crypto/external/fipsonly.
